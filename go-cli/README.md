...
boru-driven command-line client **and** interactive REPL for the AutobahnApiDe
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/autobahn-api-de-cli)
make build

# 2. See usage (words, entities, env vars)
./autobahn-api-de-cli --help
...
./autobahn-api-de-cli load 1 closure            # {id:1} shorthand
./autobahn-api-de-cli load '{id:1}' closure       # explicit match map
./autobahn-api-de-cli list electric_charging_station

# 5. Override the API base URL for a single call
AUTOBAHN_API_DE_BASE=https://api.example.com ./autobahn-api-de-cli list closure

# 6. No arguments -> interactive REPL
./autobahn-api-de-cli
autobahn-api-de> list closure
autobahn-api-de> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/autobahn-api-de-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/autobahn-api-de-cli list closure
   ```
...
```sh
./autobahn-api-de-cli load 1 closure          # scalar shorthand for {id:1}
./autobahn-api-de-cli load '{id:1}' closure     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `autobahn-api-de>`). Each line is
evaluated as its own boru expression:

```text
$ ./autobahn-api-de-cli
autobahn-api-de> list closure
autobahn-api-de> /help
autobahn-api-de> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `closure`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/autobahn-api-de-cli`. |
...
(`./autobahn-api-de-cli <expr>`) and interactively (the REPL), and expressions compose the
