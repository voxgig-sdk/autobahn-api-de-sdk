// Command AutobahnApiDe-cli is a CLI + REPL for the AutobahnApiDe SDK,
...
const prompt = "autobahn-api-de"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "closure electric_charging_station list_autobahnen parking_lorry roadwork warning webcam"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AUTOBAHN_API_DE_APIKEY (api key), AUTOBAHN_API_DE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AUTOBAHN_API_DE_APIKEY carries the API key and
	// AUTOBAHN_API_DE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AUTOBAHN_API_DE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AUTOBAHN_API_DE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAutobahnApiDeSDK(opts)
