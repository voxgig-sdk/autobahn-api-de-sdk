...
from autobahnapide_sdk.core.helpers import get_ctx_prop, to_map
...
class AutobahnApiDeContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = AutobahnApiDeControl()
        ...
        if isinstance(sp, AutobahnApiDeSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, AutobahnApiDeResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, AutobahnApiDeResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return AutobahnApiDeError(code, msg, self)
