...
class AutobahnApiDeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AutobahnApiDeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AutobahnApiDeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AutobahnApiDeTestRunner.getenv("AUTOBAHN_API_DE_TEST_LIVE")
        override = AutobahnApiDeTestRunner.getenv("AUTOBAHN_API_DE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AutobahnApiDeTestRunner.getenv(key)
        ...
        explain = AutobahnApiDeTestRunner.getenv("AUTOBAHN_API_DE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if AutobahnApiDeTestRunner._test_control is not None:
            return AutobahnApiDeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AutobahnApiDeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AutobahnApiDeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AutobahnApiDeTestRunner.load_test_control()
...
def load_env_local():
    AutobahnApiDeTestRunner.load_env_local()


def env_override(m):
    return AutobahnApiDeTestRunner.env_override(m)


def entity_data(v):
    return AutobahnApiDeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AutobahnApiDeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AutobahnApiDeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AutobahnApiDeTestRunner.load_test_control()


def live_client_options():
    return AutobahnApiDeTestRunner.live_client_options()


def live_delay_ms():
    return AutobahnApiDeTestRunner.live_delay_ms()
