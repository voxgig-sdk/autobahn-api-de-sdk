...
class TestElectricChargingStationDirect:

    def test_should_direct_list_electric_charging_station(self):
        setup = _electric_charging_station_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-electric_charging_station", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_electric_charging_station(self):
        setup = _electric_charging_station_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-electric_charging_station", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _electric_charging_station_direct_setup(mockres):
