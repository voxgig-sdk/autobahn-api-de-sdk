...
class TestWarningEntity:

    def test_should_create_instance(self):
        testsdk = AutobahnApiDeSDK.test(None, None)
        ent = testsdk.Warning(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Warning(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Warning(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _warning_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "warning." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set AUTOBAHN_API_DE_TEST_WARNING_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        warning_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.warning")))
        warning_ref01_data = None
        if len(warning_ref01_data_raw) > 0:
            warning_ref01_data = helpers.to_map(warning_ref01_data_raw[0][1])

        # LIST
        warning_ref01_ent = client.Warning(None)
        warning_ref01_match = {
            "road_id": setup["idmap"]["road01"],
        }

        warning_ref01_list_result = warning_ref01_ent.list(warning_ref01_match, None)
...
def _warning_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/warning/WarningTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("AUTOBAHN_API_DE_TEST_WARNING_ENTID"))
