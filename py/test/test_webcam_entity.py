...
class TestWebcamEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Webcam(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Webcam(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _webcam_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "webcam." + _op, "live" if _live else "unit")
            if _skip:
...
def _webcam_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/webcam/WebcamTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "AUTOBAHN_API_DE_TEST_WEBCAM_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("AUTOBAHN_API_DE_TEST_WEBCAM_ENTID"))
