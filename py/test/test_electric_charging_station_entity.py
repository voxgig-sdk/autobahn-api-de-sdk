...
class TestElectricChargingStationEntity:

    def test_should_create_instance(self):
        testsdk = AutobahnApiDeSDK.test(None, None)
        ent = testsdk.ElectricChargingStation(None)
    ...
    def test_should_stream(self):
        ...
        base = AutobahnApiDeSDK.test(seed, None)
        seen = list(base.ElectricChargingStation(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = AutobahnApiDeSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.ElectricChargingStation(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _electric_charging_station_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "electric_charging_station." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set AUTOBAHN_API_DE_TEST_ELECTRIC_CHARGING_STATION_ENTID JSON to run live")
        client = setup["client"]
        ...
        electric_charging_station_ref01_data = None
        if len(electric_charging_station_ref01_data_raw) > 0:
            electric_charging_station_ref01_data = helpers.to_map(electric_charging_station_ref01_data_raw[0][1])

        # LIST
        electric_charging_station_ref01_ent = client.ElectricChargingStation(None)
...
def _electric_charging_station_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/electric_charging_station/ElectricChargingStationTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("AUTOBAHN_API_DE_TEST_ELECTRIC_CHARGING_STATION_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("AUTOBAHN_API_DE_TEST_LIVE") == "TRUE":
        ...
        client = AutobahnApiDeSDK(helpers.to_map(merged_opts))

    _live = env.get("AUTOBAHN_API_DE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("AUTOBAHN_API_DE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
