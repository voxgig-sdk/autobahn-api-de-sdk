...
// AutobahnApiDe SDK pipeline test
//
...
{
    private static function utility(): AutobahnApiDeUtility
    {
        return new AutobahnApiDeUtility();
    }
    ...
    private static function ctx(array $over = []): AutobahnApiDeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AutobahnApiDeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AutobahnApiDeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AutobahnApiDeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AutobahnApiDeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AutobahnApiDeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AutobahnApiDeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AutobahnApiDeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AutobahnApiDeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AutobahnApiDeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AutobahnApiDeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AutobahnApiDeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AutobahnApiDeResponse([]),
            'result' => new AutobahnApiDeResult([]),
        ]);
        [, $err] = AutobahnApiDeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AutobahnApiDeSpec([]),
            'response' => null,
            'result' => new AutobahnApiDeResult([]),
        ]);
        [, $err] = AutobahnApiDeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AutobahnApiDeSpec(['step' => 's']),
            'response' => new AutobahnApiDeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AutobahnApiDeResult([]),
        ]);
        [, $err] = AutobahnApiDeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AutobahnApiDeSpec(['step' => 's']),
            'response' => new AutobahnApiDeResponse(self::resp(200, ['v' => 1])),
            'result' => new AutobahnApiDeResult([]),
        ]);
        [, $err] = AutobahnApiDeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AutobahnApiDeSpec(['step' => 's']),
            'response' => new AutobahnApiDeResponse(self::resp(200, ['v' => 2])),
            'result' => new AutobahnApiDeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AutobahnApiDeMakeResponse::call($ctx);
    ...
    {
        $preset = new AutobahnApiDeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AutobahnApiDeSpec([]),
            'response' => new AutobahnApiDeResponse([]),
            'result' => new AutobahnApiDeResult([]),
        ...
        [$got, $err] = AutobahnApiDeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AutobahnApiDeResult([])]);
        [, $err] = AutobahnApiDeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AutobahnApiDeSpec([]), 'result' => null]);
        [, $err] = AutobahnApiDeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AutobahnApiDeSpec(['step' => 's']),
            'result' => new AutobahnApiDeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AutobahnApiDeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AutobahnApiDeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AutobahnApiDeSpec(['step' => 's']),
            'result' => new AutobahnApiDeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AutobahnApiDeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AutobahnApiDeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AutobahnApiDeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AutobahnApiDeSpec([]),
            'result' => new AutobahnApiDeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AutobahnApiDeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AutobahnApiDeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AutobahnApiDeError('boom', 'boom');
        $utility->fetcher = function (AutobahnApiDeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AutobahnApiDeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AutobahnApiDeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AutobahnApiDeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AutobahnApiDeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AutobahnApiDeResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        AutobahnApiDeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AutobahnApiDeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AutobahnApiDeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AutobahnApiDeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AutobahnApiDeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AutobahnApiDeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AutobahnApiDeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AutobahnApiDeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AutobahnApiDeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AutobahnApiDeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AutobahnApiDeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AutobahnApiDeResult(['ok' => false])]);
        $this->expectException(AutobahnApiDeError::class);
        AutobahnApiDeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AutobahnApiDeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AutobahnApiDeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AutobahnApiDeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AutobahnApiDeBaseFeature();
        $b = new AutobahnApiDeBaseFeature();
        AutobahnApiDeFeatureAdd::call($ctx, $a);
        AutobahnApiDeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AutobahnApiDeBaseFeature
    {
        $f = new AutobahnApiDeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AutobahnApiDeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AutobahnApiDeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AutobahnApiDeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AutobahnApiDePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        AutobahnApiDePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AutobahnApiDeResponse(['headers' => null]),
            'result' => new AutobahnApiDeResult([]),
        ]);
        AutobahnApiDeResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        AutobahnApiDeResultBody::call($ctx);
