...
// AutobahnApiDe SDK retry feature
...
class AutobahnApiDeRetryFeature extends AutobahnApiDeBaseFeature
{
    ...
    public function init(AutobahnApiDeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AutobahnApiDeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AutobahnApiDeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AutobahnApiDeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
