...
// AutobahnApiDe SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AutobahnApiDeTestFeature extends AutobahnApiDeBaseFeature
{
    ...
    public function init(AutobahnApiDeContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AutobahnApiDeContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AutobahnApiDeParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AutobahnApiDeContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AutobahnApiDeContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AutobahnApiDeParam::call($ctx, $k);
