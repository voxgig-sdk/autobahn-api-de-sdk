...
module AutobahnApiDeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AutobahnApiDeResponse.new({})
    ...
    if fetch_err
      ...
      response = AutobahnApiDeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AutobahnApiDeResponse.new(fetched)
    else
