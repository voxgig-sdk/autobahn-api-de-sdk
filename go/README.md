# AutobahnApiDe Golang SDK



The Golang SDK for the AutobahnApiDe API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.Closure(nil)` — each with the same small set of operations (`List`, `Load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
## Install
```bash
go get github.com/voxgig-sdk/autobahn-api-de-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/autobahn-api-de-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/autobahn-api-de-sdk/go=../autobahn-api-de-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *AutobahnApiDeSDK
```
...
| `Warning` | `(data map[string]any) AutobahnApiDeEntity` | Create a Warning entity instance. |
| `Webcam` | `(data map[string]any) AutobahnApiDeEntity` | Create a Webcam entity instance. |

### Entity interface (AutobahnApiDeEntity)

All entities implement the `AutobahnApiDeEntity` interface.
...
slice):
    ...
    closure, err := client.Closure(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // closure is the returned record
...
Create an instance: `closure := client.Closure(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `coordinate` | `map[string]any` | Beginn des betroffenen Streckenabschnitts (Koordinaten in WGS84). |
| `description` | `[]any` |  |
| `display_type` | `string` |  |
...
Create an instance: `roadwork := client.Roadwork(nil)`
...
Create an instance: `warning := client.Warning(nil)`
...
Create an instance: `webcam := client.Webcam(nil)`
...
```
github.com/voxgig-sdk/autobahn-api-de-sdk/go/
├── autobahn-api-de.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/autobahn-api-de-sdk/go`) re-exports everything needed
