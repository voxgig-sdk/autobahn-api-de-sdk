...
func TestWebcamEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Webcam(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Webcam(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		webcamRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.webcam")))
...
func webcamBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("AUTOBAHN_API_DE_TEST_WEBCAM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["AUTOBAHN_API_DE_TEST_WEBCAM_ENTID"])
