...
import (
	...
	omni "github.com/voxgig-sdk/autobahn-api-de-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AutobahnApiDeError); is {
				out := map[string]any{
					"name":    "AutobahnApiDeError",
...
func omniCtx(arg any, client *sdk.AutobahnApiDeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AutobahnApiDeSDK); is {
			client = live
