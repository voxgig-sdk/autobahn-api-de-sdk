...
func TestRoadworkEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set AUTOBAHNAPIDE_TEST_ROADWORK_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		roadworkRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.roadwork", setup.data)))
...
func roadworkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("AUTOBAHNAPIDE_TEST_ROADWORK_ENTID")
