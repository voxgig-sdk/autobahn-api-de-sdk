...
func TestListAutobahnenEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.ListAutobahnen(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ListAutobahnen(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		listAutobahnenRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.list_autobahnen")))
...
func list_autobahnenBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("AUTOBAHN_API_DE_TEST_LIST_AUTOBAHNEN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["AUTOBAHN_API_DE_TEST_LIST_AUTOBAHNEN_ENTID"])
