...
func TestElectricChargingStationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		electricChargingStationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.electric_charging_station")))
...
func electric_charging_stationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("AUTOBAHN_API_DE_TEST_ELECTRIC_CHARGING_STATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["AUTOBAHN_API_DE_TEST_ELECTRIC_CHARGING_STATION_ENTID"])
