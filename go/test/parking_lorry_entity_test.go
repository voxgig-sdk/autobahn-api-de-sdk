...
func TestParkingLorryEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.ParkingLorry(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ParkingLorry(nil).Stream("list", nil, nil) {
...
func parking_lorryBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("AUTOBAHN_API_DE_TEST_PARKING_LORRY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["AUTOBAHN_API_DE_TEST_PARKING_LORRY_ENTID"])
