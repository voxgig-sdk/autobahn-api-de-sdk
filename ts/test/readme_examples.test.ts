...
//     (\`new AutobahnApiDeSDK(...)\` / \`AutobahnApiDeSDK.test(...)\` become
//     \`AutobahnApiDeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AutobahnApiDeSDK'
...
const TEST_SEED = {"entity":{"closure":{"test01":{"id":"test01"}},"electric_charging_station":{"test01":{"id":"test01"}},"list_autobahnen":{"test01":{"id":"test01"}},"parking_lorry":{"test01":{"id":"test01"}},"roadwork":{"test01":{"id":"test01"}},"warning":{"test01":{"id":"test01"}},"webcam":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AutobahnApiDeSDK(...)\` and any
// \`AutobahnApiDeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AutobahnApiDeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AutobahnApiDeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
