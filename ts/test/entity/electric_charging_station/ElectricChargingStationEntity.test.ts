...
describe('ElectricChargingStationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'electric_charging_station.' + op, live)) return
    }
    ...
    let electric_charging_station_ref01_data = Object.values(setup.data.existing.electric_charging_station)[0] as any

    // LIST
    const electric_charging_station_ref01_ent = client.ElectricChargingStation()
    const electric_charging_station_ref01_match: any = {}
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/electric_charging_station/ElectricChargingStationTestData.json')
  ...
  let idmap = transform(
    ['electric_charging_station01','electric_charging_station02','electric_charging_station03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
