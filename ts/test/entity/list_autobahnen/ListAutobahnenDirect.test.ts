...
describe('ListAutobahnenDirect', async () => {
  ...
  test('direct-list-list_autobahnen', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-list_autobahnen', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['AUTOBAHN_API_DE_TEST_LIST_AUTOBAHNEN_ENTID']
