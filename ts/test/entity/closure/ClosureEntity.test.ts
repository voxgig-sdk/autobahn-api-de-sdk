...
describe('ClosureEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'closure.' + op, live)) return
    }
    ...
    let closure_ref01_data = Object.values(setup.data.existing.closure)[0] as any

    // LIST
    const closure_ref01_ent = client.Closure()
    const closure_ref01_match: any = {}
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/closure/ClosureTestData.json')
