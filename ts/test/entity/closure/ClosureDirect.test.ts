...
describe('ClosureDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new AutobahnApiDeSDK({
  ...
  test('direct-load-closure', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-closure', setup.live)) return
  ...
  test('direct-list-closure', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-closure', setup.live)) return
    if (skipIfMissingIds(t, setup, ["road01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['AUTOBAHN_API_DE_TEST_CLOSURE_ENTID']
