...
describe('RoadworkEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'roadwork.' + op, live)) return
    }
    ...
    let roadwork_ref01_data = Object.values(setup.data.existing.roadwork)[0] as any

    // LIST
    const roadwork_ref01_ent = client.Roadwork()
    const roadwork_ref01_match: any = {}
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/roadwork/RoadworkTestData.json')
  ...
  let idmap = transform(
    ['roadwork01','roadwork02','roadwork03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
