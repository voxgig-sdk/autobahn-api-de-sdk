-- AutobahnApiDe SDK Webcam entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local WebcamEntity = {}
WebcamEntity.__index = WebcamEntity


function WebcamEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebcamEntity)
  self._name = "webcam"
  self._client = client
...
function WebcamEntity:get_name()
  return self._name
end


function WebcamEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebcamEntity.new(self._client, opts)
end
...
function WebcamEntity:mark_deleted()
  self._deleted = true
end


function WebcamEntity:deleted()
  return true == self._deleted
end


function WebcamEntity:data_set(args)
  if args ~= nil then
...
function WebcamEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebcamEntity:match_set(args)
  if args ~= nil then
...
function WebcamEntity:match_get()
...
function WebcamEntity:stream(action, args, callopts)
...
function WebcamEntity:_run_op(ctx, post_done)
  local utility = self._utility
