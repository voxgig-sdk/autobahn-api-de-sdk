...
local ClosureEntity = {}
ClosureEntity.__index = ClosureEntity


function ClosureEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClosureEntity)
  self._name = "closure"
  self._client = client
...
function ClosureEntity:get_name()
  return self._name
end


function ClosureEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClosureEntity.new(self._client, opts)
end
...
function ClosureEntity:mark_deleted()
  self._deleted = true
end


function ClosureEntity:deleted()
  return true == self._deleted
end


function ClosureEntity:data_set(args)
  if args ~= nil then
...
function ClosureEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClosureEntity:match_set(args)
  if args ~= nil then
...
function ClosureEntity:match_get()
...
function ClosureEntity:stream(action, args, callopts)
...
---@param reqmatch ClosureLoadMatch
---@param ctrl? table
---@return Closure
---@return string? err
function ClosureEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ClosureListMatch
---@param ctrl? table
---@return Closure[]
---@return string? err
function ClosureEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ClosureEntity:_run_op(ctx, post_done)
  local utility = self._utility
