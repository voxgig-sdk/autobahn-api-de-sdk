...
local RoadworkEntity = {}
RoadworkEntity.__index = RoadworkEntity


function RoadworkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RoadworkEntity)
  self._name = "roadwork"
  self._client = client
...
function RoadworkEntity:get_name()
  return self._name
end


function RoadworkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RoadworkEntity.new(self._client, opts)
end
...
function RoadworkEntity:mark_deleted()
  self._deleted = true
end


function RoadworkEntity:deleted()
  return true == self._deleted
end


function RoadworkEntity:data_set(args)
  if args ~= nil then
...
function RoadworkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RoadworkEntity:match_set(args)
  if args ~= nil then
...
function RoadworkEntity:match_get()
...
function RoadworkEntity:stream(action, args, callopts)
...
function RoadworkEntity:_run_op(ctx, post_done)
  local utility = self._utility
