...
describe("ParkingLorryDirect", function()
  it("should direct-list-parking_lorry", function()
    local setup = parking_lorry_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-parking_lorry", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"road01"}) do
  ...
  it("should direct-load-parking_lorry", function()
    local setup = parking_lorry_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-parking_lorry", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function parking_lorry_direct_setup(mockres)
