...
describe("WarningDirect", function()
  it("should direct-list-warning", function()
    local setup = warning_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-warning", setup.live and "live" or "unit")
  ...
  it("should direct-load-warning", function()
    local setup = warning_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-warning", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function warning_direct_setup(mockres)
