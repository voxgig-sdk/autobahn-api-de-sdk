...
describe("WebcamDirect", function()
  it("should direct-list-webcam", function()
    local setup = webcam_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-webcam", setup.live and "live" or "unit")
  ...
  it("should direct-load-webcam", function()
    local setup = webcam_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-webcam", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function webcam_direct_setup(mockres)
