...
describe("ElectricChargingStationDirect", function()
  it("should direct-list-electric_charging_station", function()
    local setup = electric_charging_station_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-electric_charging_station", setup.live and "live" or "unit")
  ...
  it("should direct-load-electric_charging_station", function()
    local setup = electric_charging_station_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-electric_charging_station", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
