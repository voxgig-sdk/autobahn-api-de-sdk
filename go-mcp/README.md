...
[MCP](https://modelcontextprotocol.io) server exposing the AutobahnApiDe SDK as
two agent tools — `autobahn-api-de_list` and `autobahn-api-de_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/autobahn-api-de-mcp)
make build

# 2. Provide credentials via the environment
export AUTOBAHN_API_DE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user autobahn-api-de \
  -- /absolute/path/to/autobahn-api-de-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./autobahn-api-de-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// autobahn-api-de_list: first page of records
{ "entity": "closure" }
{ "entity": "closure", "query": { } }

// autobahn-api-de_load: one record by id
{ "entity": "closure", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/autobahn-api-de-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user autobahn-api-de \
     -- "$PWD"/dist/*/autobahn-api-de-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `autobahn-api-de_list` and `autobahn-api-de_load` tools now appear
   in new sessions. Ask the agent to *"list closure using autobahn-api-de"*
   and it calls `autobahn-api-de_list` with `{"entity":"closure"}`.
...
```sh
export AUTOBAHN_API_DE_APIKEY=sk_live_xxx            # API key
export AUTOBAHN_API_DE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `autobahn-api-de_list` tool
...
### Call the `autobahn-api-de_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `autobahn-api-de_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `autobahn-api-de_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AUTOBAHN_API_DE_APIKEY` | API key sent with every request. |
| `AUTOBAHN_API_DE_BASE` | Optional override of the API base URL. |
...
```sh
./autobahn-api-de-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"autobahn-api-de_load","arguments":{"entity":"closure","query":{"id":1}}}}'
```
