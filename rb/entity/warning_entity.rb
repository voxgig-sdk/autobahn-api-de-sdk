...
class WarningEntity
  ...
  def make
    opts = @_entopts.dup
    WarningEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Warning.load works with no args).
  ...
  #
  # @param reqmatch [WarningListMatch, Hash, nil] match filter (any subset of
  #   Warning fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
