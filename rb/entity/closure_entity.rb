...
class ClosureEntity
  ...
  def make
    opts = @_entopts.dup
    ClosureEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Closure.load works with no args).
  ...
  #
  # @param reqmatch [ClosureListMatch, Hash, nil] match filter (any subset of
  #   Closure fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
