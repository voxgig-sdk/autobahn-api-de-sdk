...
class WebcamEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set AUTOBAHN_API_DE_TEST_WEBCAM_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    webcam_ref01_data_dt0_load_result = Helpers.to_map(webcam_ref01_data_dt0_loaded.respond_to?(:data_get) ? webcam_ref01_data_dt0_loaded.data_get : webcam_ref01_data_dt0_loaded)
    assert !webcam_ref01_data_dt0_load_result.nil?
    assert_equal webcam_ref01_data_dt0_load_result["id"], webcam_ref01_data["id"]
...
def webcam_basic_setup(extra)
  ...
  entid_env_raw = ENV["AUTOBAHN_API_DE_TEST_WEBCAM_ENTID"]
