...
class ListAutobahnenDirectTest < Minitest::Test
  def test_direct_list_list_autobahnen
    setup = list_autobahnen_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-list_autobahnen", setup[:live] ? "live" : "unit")
    if _should_skip
...
def list_autobahnen_direct_setup(mockres)
