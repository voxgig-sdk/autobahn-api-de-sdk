...
class RoadworkEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Roadwork(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Roadwork(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = roadwork_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "roadwork." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set AUTOBAHN_API_DE_TEST_ROADWORK_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    roadwork_ref01_data = nil
    if roadwork_ref01_data_raw.length > 0
      roadwork_ref01_data = Helpers.to_map(roadwork_ref01_data_raw[0][1])
    end

    # LIST
    roadwork_ref01_ent = client.Roadwork(nil)
...
def roadwork_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "roadwork", "RoadworkTestData.json")
  ...
  entid_env_raw = ENV["AUTOBAHN_API_DE_TEST_ROADWORK_ENTID"]
