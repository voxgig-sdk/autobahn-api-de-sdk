...
class ParkingLorryEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set AUTOBAHN_API_DE_TEST_PARKING_LORRY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    parking_lorry_ref01_data_dt0_load_result = Helpers.to_map(parking_lorry_ref01_data_dt0_loaded.respond_to?(:data_get) ? parking_lorry_ref01_data_dt0_loaded.data_get : parking_lorry_ref01_data_dt0_loaded)
    assert !parking_lorry_ref01_data_dt0_load_result.nil?
    assert_equal parking_lorry_ref01_data_dt0_load_result["id"], parking_lorry_ref01_data["id"]
...
def parking_lorry_basic_setup(extra)
  ...
  entid_env_raw = ENV["AUTOBAHN_API_DE_TEST_PARKING_LORRY_ENTID"]
