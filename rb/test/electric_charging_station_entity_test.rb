...
class ElectricChargingStationEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.ElectricChargingStation(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.ElectricChargingStation(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = electric_charging_station_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "electric_charging_station." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set AUTOBAHN_API_DE_TEST_ELECTRIC_CHARGING_STATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    electric_charging_station_ref01_data = nil
    if electric_charging_station_ref01_data_raw.length > 0
      electric_charging_station_ref01_data = Helpers.to_map(electric_charging_station_ref01_data_raw[0][1])
    end

    # LIST
    electric_charging_station_ref01_ent = client.ElectricChargingStation(nil)
...
def electric_charging_station_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "electric_charging_station", "ElectricChargingStationTestData.json")
  ...
  entid_env_raw = ENV["AUTOBAHN_API_DE_TEST_ELECTRIC_CHARGING_STATION_ENTID"]
