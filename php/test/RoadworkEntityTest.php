...
class RoadworkEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = AutobahnApiDeSDK::test(null, null);
        $ent = $testsdk->Roadwork(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Roadwork(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Roadwork(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = roadwork_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "roadwork." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set AUTOBAHN_API_DE_TEST_ROADWORK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $roadwork_ref01_data = null;
        if (count($roadwork_ref01_data_raw) > 0) {
            $roadwork_ref01_data = Helpers::to_map($roadwork_ref01_data_raw[0][1]);
        }

        // LIST
        $roadwork_ref01_ent = $client->Roadwork(null);
...
function roadwork_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/roadwork/RoadworkTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["roadwork01", "roadwork02", "roadwork03", "road01"] as $k) {
    ...
    $entid_env_raw = getenv("AUTOBAHN_API_DE_TEST_ROADWORK_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["AUTOBAHN_API_DE_TEST_ROADWORK_ENTID"]);
