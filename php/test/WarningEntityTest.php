...
class WarningEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = AutobahnApiDeSDK::test(null, null);
        $ent = $testsdk->Warning(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Warning(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Warning(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = warning_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "warning." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $warning_ref01_data = null;
        if (count($warning_ref01_data_raw) > 0) {
            $warning_ref01_data = Helpers::to_map($warning_ref01_data_raw[0][1]);
        }

        // LIST
        $warning_ref01_ent = $client->Warning(null);
        $warning_ref01_match = [
...
function warning_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/warning/WarningTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["warning01", "warning02", "warning03", "road01"] as $k) {
