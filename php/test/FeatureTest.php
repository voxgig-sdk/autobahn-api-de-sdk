...
// AutobahnApiDe SDK feature test
//
...
class FtCtrl extends AutobahnApiDeControl
{
...
class FtClient
{
    ...
    private AutobahnApiDeUtility $utility;

    public function __construct(AutobahnApiDeUtility $utility, array $options)
    {
    ...
    public function get_utility(): AutobahnApiDeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AutobahnApiDeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AutobahnApiDeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AutobahnApiDeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AutobahnApiDeConfig::make_feature($name);
    ...
    private static function buildUrl(AutobahnApiDeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AutobahnApiDeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AutobahnApiDeResponse($fetched);
        }
    ...
    private function fail(AutobahnApiDeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AutobahnApiDeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
