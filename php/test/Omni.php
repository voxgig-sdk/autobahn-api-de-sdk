...
//    only - a typed AutobahnApiDeContext there would make every ctx
...
final class AutobahnApiDeOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AutobahnApiDeContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new AutobahnApiDeSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new AutobahnApiDeResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new AutobahnApiDeError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new AutobahnApiDeResponse($clean['response']);
    ...
    public static function err_from_map($m): ?AutobahnApiDeError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AutobahnApiDeError(is_string($code) ? $code : '', $msg);
    }
