...
// AutobahnApiDe SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AutobahnApiDeOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AutobahnApiDeOmni::structfix($now, Struct::undef()))
                        !== json_encode(AutobahnApiDeOmni::structfix($val, Struct::undef()))) {
